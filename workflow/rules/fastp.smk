...
rule fastp:
    ...
    output:   
        trimmed_1           = OUTPUT_DIR + "qc/fastp/{sample}_trimmed_R1.fastq.gz",
        trimmed_2           = OUTPUT_DIR + "qc/fastp/{sample}_trimmed_R2.fastq.gz",
        html                = OUTPUT_DIR + "qc/fastp/logs/{sample}_trimmed_fastp.html",
        json                = OUTPUT_DIR + "qc/fastp/logs/{sample}_trimmed_fastp.json"        
    message:
    ...
    shell:
        "mkdir -p {OUTPUT_DIR}/qc/fastp/logs; "
        "fastp "
        "-i {input.read_1} "
        "-I {input.read_2} "
        "-o {output.trimmed_1} "
